...
import static com.yansb.admin.api.domain.Fixture.Videos.resource;
...
class DefaultMediaResourceGatewayTest {
    ...
    public void givenValidVideoId_whenCallsGetResource_shouldReturnIt() {
        ...
        storageService.store("videoId-%s/type-%s".formatted(videoOne.getValue(), VideoMediaType.TRAILER.name()), expectedResource);
        storageService.store("videoId-%s/type-%s".formatted(videoOne.getValue(), VideoMediaType.BANNER.name()), expectedResource);
